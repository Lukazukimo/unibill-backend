 ...
 * known-pwned passwords (via GOTRUE_PASSWORD_HIBP_ENABLED=true) AND accepts
 * a strong unique password. Run nightly and on every PR that touches the
 * auth config — see .github/workflows/auth-hibp.yml.
 *
 ...
 *
 * Exit codes:
 *   0 — every pwned password rejected via HIBP AND strong password accepted.
...
function isHibpRejection(outcome: SignupOutcome): boolean {
  ...
  const reasons = (body.weak_password?.reasons ?? []).map((r) =>
    r.toLowerCase()
  );
...
async function main(): Promise<never> {
  ...
  console.log(
    `[verify-hibp] running ${PWNED_PASSWORDS.length} pwned-rejection ` +
      `cases + 1 strong-acceptance case...`,
  );
